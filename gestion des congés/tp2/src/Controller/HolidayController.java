...
public class HolidayController {
    ...
    private void addHoliday() {
        ...
        if (daysBetween <= 0) {
            View.afficherMessageErreur("Les dates de debut et de fin sont invalides.");
            return;
        }

        // V�rifier si le solde de conge est suffisant
        if (targetEmploye.getSolde() < daysBetween) {
            View.afficherMessageErreur("Le solde de conge de l'employe est insuffisant.");
            return;
        }

        try {
            // Ajouter la demande de conge
            boolean addReussi = model_holiday.addHoliday(0, id_employe, startDate, endDate, type, targetEmploye);

            if (addReussi) {
                // R�duire le solde de conge aprés l'ajout réussi
                targetEmploye.setSolde(targetEmploye.getSolde() - (int) daysBetween);
                View.afficherMessageSucces("Holiday a bien ajoutee.");
            } else {
                View.afficherMessageErreur("Holiday n'a pas ajoutee.");
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void updateHolidaybyselect(){
        ...
        try{
            ...
            test = true;
             ...
             View.afficherMessageErreur("Erreur lors de la recuperation des données");
        }
    }
    ...
    private void updateHoliday(){
        if (!test) {
            View.afficherMessageErreur("Veuillez d'abord s�lectionner une ligne a modifier.");
            return;
        }
        try {
            ...
            if (updateSuccessful) {
                test = false; 
                View.afficherMessageSucces("Holiday modifiee avec succes.");
                displayHoliday();
                View.viderChamps_ho();
            } else {
